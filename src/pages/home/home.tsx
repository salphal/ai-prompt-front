...
const Home: ForwardRefRenderFunction<HomeRef, HomeProps> = (
    ...
    render: (v: any, record: any) => {
      return (
        <div>
          ...
          <Button size={'small'} onClick={() => handlePromptEventAspect('delete', record)}>
            Delete
          </Button>
        </div>
      );
  ...
  const { tableColumns, tableColumnKeys, setTableColumnBlackKeys, rowData } = useTableColumns({
  ...
  useLayoutEffect(() => {
    setLoading(true);
    const countDown = new Countdown(2, (duration) => {
  ...
  const dataSourceByFilter = useMemo(
    () => () => {
      ...
      return dataSource.filter(
        (rowData: any) =>
          typeof rowData[queryKey] === 'string' &&
          rowData[queryKey].toLowerCase().indexOf(queryString) !== -1,
      );
    },
    [formData, dataSource],
  );
        ...
        <div className={'flex flex-col justify-between h-full'}>
          <div className={classNames(['flex', 'flex-row', 'justify-between', 'items-center'])}>
            ...
            <div className={classNames(['flex', 'justify-end', 'items-center', 'h-16'])}>
              ...
              >
                {isEditable ? 'Editable' : 'UnEditable'}
              </Button>
              <Button
                className={'mr-3'}
                icon={<DiffOutlined />}
                ...
                content={
                  <div className={classNames(['pb-6'])}>
                    <div className={classNames(['flex', 'justify-end', 'm-3'])}>
                      <Button size={'small'} onClick={selectedAllOnChange}>
                        Select all
                      </Button>
                    </div>
                    <Checkbox.Group
              ...
              >
                <Button
                  icon={
                    columnKeys.length - 1 === columnFilterKeys.length ||
