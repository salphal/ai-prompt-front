...
const EditPrompt: React.FC<EditPromptProps> = (props: EditPromptProps) => {
  ...
  const { columnKeysOptions, contextKeysOptions, contextData } = usePromptStore(
    useShallow((state: any) => state),
  );

  const { promptFormData } = useEditPromptStore(useShallow((state: any) => state));
    ...
    () => () => {
      ...
      return messageList.map((v: any, i: number) => {
        ...
        return {
          ...
          ...v,
          render: () => (
            <div key={`prompt-message-${i}`} className={classNames(['flex', 'justify-between'])}>
              <div className={classNames(['flex-1'])}>
                <PromptMessage value={v} onChange={(val: any) => promptMessageOnChange(val, id)} />
              </div>
              <div
              ...
              >
                {[
                  ...
                  [
                    <VerticalAlignTopOutlined className={classNames(['text-blue-500'])} />,
                    'moveToPrev',
                  ],
                  [
                    <VerticalAlignBottomOutlined className={classNames(['text-blue-500'])} />,
                    'moveToNext',
                  ],
                ].map(([icon, type], i) => (
                  <div
  ...
  return (
    <React.Fragment>
      <div className={classNames(['h-full', 'flex', 'flex-col', 'flex-nowrap'])}>
        ...
        <DraggableList dataSource={promptList()} setDataSource={setMessageList} />
        ...
        >
          ...
          <Button type={'primary'} onClick={() => handleEditPromptEventAspect('save')}>
            Merge
          </Button>
        </div>
      </div>
